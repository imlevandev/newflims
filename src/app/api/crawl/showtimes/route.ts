...
import type { ShowtimeCrawlRequestDto } from "@/server/modules/movies/dto/crawl.dto";

const moviesCrawlService = new MoviesCrawlService();

export async function POST(request: Request) {
  const headerSecret = request.headers.get("x-crawl-secret");
  const body = (await request.json().catch(() => ({}))) as ShowtimeCrawlRequestDto;
  ...
  return handleRoute(() => {
    ...
    return moviesCrawlService.crawlByShowtimes(body);
  });
}
